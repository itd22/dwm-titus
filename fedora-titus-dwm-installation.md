...
- If you only have the dwm-titus ISO built from a netinst image and do not want to use the internet during install: run a simple HTTP server on your host that serves a full Fedora DVD tree (or the hos[...]
...
- If ISO is netinst/live (no `repodata/`) and you do not want the installer to talk to the public internet:
  ...
  - Option B — Allow the installer to use the network (easiest if you do not need strict offline): leave default network repos enabled. The dwm-titus Kickstart already enables Fedora and RPM Fusion [...]
...
- NOTE: installer environment (the "kind" or "spin" you select — e.g. Server, Fedora Workstation (GNOME), KDE Plasma, Xfce, LXDE) affects what is installed by default. To have dwm functional from first boot you have two main approaches:

  - Recommended (minimal, controlled):
    - Choose "Minimal Install" for the Base Environment.
    - Enable "Customize now" and explicitly add the X/graphical packages listed below plus a lightweight display manager such as `lightdm`.
    - This keeps the system small and ensures only packages you need for a functional dwm session are present.
    - If you want dwm to be immediately selectable from a greeter on first boot, install `lightdm` (or another DM) and ensure the dwm session file (`dwm.desktop`) is installed during `%post` or the first-boot setup.

  - Alternative (desktop spin / ready-to-use but heavier):
    - Choose a desktop spin such as "Fedora Workstation" (GNOME), "KDE Plasma Workspaces", "Xfce", or "LXDE" if you prefer the installer to provision a full graphical desktop automatically.
    - These spins install a display manager and a full desktop environment; dwm can be added alongside them and selected from the session chooser.
    - Workstation (GNOME) will be the heaviest option; Xfce or LXDE spins are lighter and closer in footprint to a minimal graphical install.

  - Server install:
    - The Server environment does not include X by default. If you pick Server, you must add the X packages and a display manager manually in the Software Selection step to run dwm on first boot.
...
- Add the X/graphical groups or the following packages (group names and package availability depend on Fedora version):
  ...
  - Display manager (choose one to get a greeter/session chooser on first boot):
    - lightdm (recommended for minimal installs)
    - gdm (GNOME default; heavier)
    - sddm (KDE/Plasma default; heavier)
...
- Installing a display manager during install makes dwm immediately selectable from a greeter on first boot; otherwise you'll need to enable/start a DM or configure `.xinitrc` manually.

If you want the installer to make dwm usable immediately after first boot, follow the "Recommended (minimal, controlled)" flow above: Minimal Install + `@base-x` + `lightdm` + dwm build/install in %post or a first-boot script.
...
- The Kickstart (`dwm-fedora.ks`) includes network repo metalinks by default. If you want the installer to *not* use the network, you must ensure the package repository the installer uses is the local[...]
- After the base packages are installed you can open a shell in the installer (Troubleshooting → Shell) and inspect `/run/install/repo/dwm-titus` to run the included installer scripts manually or to[...]

7) Post-install steps (make dwm-titus the active session)
- If Kickstart ran and installed all packages listed in the dwm-fedora.ks `%packages`, the system may still need the dwm binaries built/installed (depends on what the ISO/Kickstart package list includ[...]
...
- If you want the Kickstart to perform everything automatically, boot the ISO entry that applies the Kickstart (the project build script usually installs a boot menu entry that uses the embedded Kicks[...]
...
Summary
- Best offline experience: use a DVD/Everything ISO (or rebuild the dwm-titus ISO with a DVD as input). In that case choose "Local media" as the Installation Source and set Software Selection to "Mini[...]
- If your dwm-titus ISO is netinst-based, serve a full DVD tree from the host via HTTP and point Anaconda to `http://10.0.2.2:8000/` (or allow network access to Fedora mirrors and let the Kickstart us[...]
